...
contextBridge.exposeInMainWorld("gitdeck", {
  ...
  discardFile: (repoPath: string, filePath: string, source: "unstaged" | "staged") => ipcRenderer.invoke(Channels.discardFile, repoPath, filePath, source),
  ...
  createTag: (repoPath: string, tag: string, startPoint?: string) => ipcRenderer.invoke(Channels.createTag, repoPath, tag, startPoint),
  pushTag: (repoPath: string, tag: string) => ipcRenderer.invoke(Channels.pushTag, repoPath, tag),
